    ...
    context "on #{os}" do
      ...
      context 'with winbind separator removed' do
        ...
        it 'does not include winbind separator smb.conf' do
          is_expected.to contain_file('/etc/samba/smb.conf').without_content(%r{winbind\sseparator\s+ = \+})
        end
      end
      ...
      context 'with default winbind separator' do
        ...
        it 'includes default winbind separator in smb.conf' do
          is_expected.to contain_file('/etc/samba/smb.conf').with_content(%r{winbind\sseparator\s+ = \+})
        end
      end
